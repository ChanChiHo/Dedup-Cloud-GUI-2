	...
	public void initIndex() {
		...
		try {
			...
			if (file.exists()) {
				...
				System.out.println("Outdated Session data clear.");
			} else {
				createIndex();
			}
		} catch (Exception e) {
	...
	public void run(){
		while (true) {
			...
			try {
				...
				System.out.println("Server - Accept Connection ["+LocalDateTime.now()+"]");
				
				LocalDateTime nowTime = LocalDateTime.now(); 
				//TODO Change the refresh minutes to 1 min
				if (nowTime.minusMinutes(1).compareTo(lastTime) >= 0) {
					System.out.println("Maintance : "+nowTime+" - Clearing unused session");
					this.index.removeTimeoutSession(lastTime);
					try {
						writeIndex();
					} catch (IOException e) {
				...
				case REQUEST_HEARTBEAT:
					...
					this.dos.writeInt(Server.HEARTBEAT);
				...
				case REQUEST_LOGIN:
					...
					System.out.println(
							"[FROM Client] {Username, passwordHash} :" + loginUsername + "," + loginPasswordHash);

					if (!index.hasUser(loginUsername)) {
						System.out.println("Server - Username not correct.");
						this.dos.writeInt(Server.NO_USERNAME);
						this.socket.close();
						System.out.println("[Server] End Connection.");
						break;
					...
					if (index.checkUserPass(loginUsername, loginPasswordHash)) {
						...
						this.dos.writeInt(Server.LOGIN_SUCCESS);
						...
						this.dos.writeInt(Server.PASSWORD_WRONG);
					}
				...
				case REQUEST_UPLOAD:
					...
					System.out.println("[Client] Receive Session Key = " + sessionKey
							+ ", start checking the user with this key..");

					if (!this.index.hasSession(sessionKey)) {
						System.out.println("Server - No user with this session key.");
						this.dos.writeInt(Server.NOT_AUTHORIZED);
						break;
					...
					if (msgCode == ALLOW_UPLOAD) {
						...
						if (msgCode == REQUEST_UPLOAD_END) {
							...
							System.out
									.println("Number of unique physical chunks in storage:" + this.index.physicalChunk);
							this.dos.writeUTF(
									"Number of unique physical chunks in storage:" + this.index.physicalChunk);
				...
				case REQUEST_LIST:
					...
					if (!this.index.hasSession(sessionKey)) {
						...
						username = this.index.getUserFromKey(sessionKey);
						System.out.println("Server - Confirm Username = " + username);
						System.out.println("Server - "+username+" Timeout : "+this.index.sessionTimeoutList.get(sessionKey));
						// this.dos.writeInt(Server.AUTHORIZED);
					}
